...
def select_dronedb_assets(assets: list[MediaAsset]) -> list[MediaAsset]:
    ...
    if complete_groups < 2:
        raise ValueError(
            "DroneDB handoff requires at least two complete M3M "
            "RGB + Green/Red/RedEdge/NIR capture groups"
        )
    return selected
...
def build_dronedb_handoff(
        ...
        assets.append(
            {
                ...
                "remote_path": _dronedb_remote_path(
                    job.input_prefix,
                    item.relative_path,
                ),
    ...
    complete_groups = sum(
        1 for kinds in grouped.values() if required.issubset(kinds)
    )
    if complete_groups < 2:
        raise ValueError("Frozen DroneDB handoff no longer contains two complete M3M groups")

    return {
        "schema_version": M3M_DRONEDB_HANDOFF_SCHEMA,
        ...
        "assets": assets,
        "note": (
            "DroneDB receives byte-for-byte verified M3M originals plus M3-Cloud "
            "provenance. Downstream photogrammetry, reflectance and vegetation-index "
            "processing remain DroneDB-side tasks."
        ),
    }
...
class ProcessingManager:
    ...
    async def dronedb_handoff(self, job_id: uuid.UUID) -> dict[str, object]:
        async with self.sessions() as session:
            ...
            return build_dronedb_handoff(job, frozen)
    ...
    async def _run_job(self, job_id: uuid.UUID) -> None:
        ...
        if kind == "WEBODM":
            ...
            await self._fail_job(
                job_id,
                ValueError(f"Unsupported queued processing kind: {kind}"),
            )
    ...
    async def _run_dronedb_job(self, job_id: uuid.UUID) -> None:
        ...
        try:
            async with self.sessions() as session:
                ...
                for source_path, item in zip(paths, frozen, strict=True):
                ...
                org_slug = str(
                    _job_option(job, "dronedb_org") or self.dronedb_org
                )
            ...
            total = len(frozen)
            for index, (item, source_path) in enumerate(
                zip(frozen, paths, strict=True),
                start=1,
            ):
                remote_path = _dronedb_remote_path(
                    str(handoff["input_prefix"]),
                    item.relative_path,
                )
                await asyncio.to_thread(
                    client.upload_file,
