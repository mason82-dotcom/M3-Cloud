...
def test_m3_family_capabilities_stay_separate():
    ...
    m4t = capabilities_for(AircraftPlatform.M4T)
    ...
    assert m4t.thermal is True and m4t.wide is True and m4t.zoom is True and m4t.lrf is True
    assert "M4T_WIDE" in m4t.capture_profiles
    assert "M4T_THERMAL" in m4t.capture_profiles
...
def test_camera_type_policy_matches_lyrebird_and_fails_closed():
    ...
    assert platform_from_camera_type("M4T") == AircraftPlatform.M4T
...
def test_thermal_flag_is_not_model_identity():
    ...
    assert platform_from_explicit_model("Matrice 4T") == AircraftPlatform.M4T
