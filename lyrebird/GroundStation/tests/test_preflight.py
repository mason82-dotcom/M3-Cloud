...
def snapshot(**updates):
    data = {
        ...
        "gimbal": {"valid": True},
        "rtk": {
            "enabled": True, "connected": True, "healthy": True,
            "fix": "FIXED", "ageMs": 250, "source": "CUSTOM_NETWORK_SERVICE",
        },
    }
    data.update(updates)
    return data
...
def mission_items():
    return [
        {
            "seq": 0, "command": 206, "frame": 2, "autocontinue": True,
            "param1": 15.8, "param2": 0.0, "param3": 0.0, "param4": "NaN",
            "latitude": 0.0, "longitude": 0.0, "altitude": 0.0, "missionType": 0,
        },
        {
            "seq": 1, "command": 16, "frame": 6, "autocontinue": True,
            "param1": 0.0, "param2": 0.0, "param3": 0.0, "param4": "NaN",
            "latitude": 49.1, "longitude": 8.6, "altitude": 74.8, "missionType": 0,
        },
        {
            "seq": 2, "command": 16, "frame": 6, "autocontinue": True,
            "param1": 0.0, "param2": 0.0, "param3": 0.0, "param4": "NaN",
            "latitude": 49.2, "longitude": 8.6, "altitude": 74.8, "missionType": 0,
        },
        {
            "seq": 3, "command": 206, "frame": 2, "autocontinue": True,
            "param1": 0.0, "param2": 0.0, "param3": 0.0, "param4": "NaN",
            "latitude": 0.0, "longitude": 0.0, "altitude": 0.0, "missionType": 0,
        },
    ]
...
def wiretap(tmp_path: Path, items=None):
    ...
    rows = [{
        "direction": "VSM_TO_RC", "message": "MISSION_COUNT",
        "count": len(items), "blocked": False, "epochNs": (NOW - 12_000) * 1_000_000,
    }]
    for item in items:
        rows.append({
            **item, "direction": "VSM_TO_RC", "message": "MISSION_ITEM_INT",
            "missionSeq": item["seq"], "blocked": False,
            "epochNs": (NOW - 11_000 + item["seq"]) * 1_000_000,
        })
    rows.append({
        "direction": "RC_TO_VSM", "message": "MISSION_ACK", "result": 0,
...
def test_all_required_gates_can_produce_go(tmp_path):
    report = evaluate_preflight(
        rc_host="rc", snapshot=snapshot(), mission_trace=trace(),
        wiretap_path=wiretap(tmp_path), expected_platform="M3E",
        expected_capture_profile="M3E_MAPPING", now_epoch_ms=NOW + 100,
    )
...
def test_missing_wiretap_forces_no_go():
    report = evaluate_preflight(
        rc_host="rc", snapshot=snapshot(), mission_trace=trace(),
        wiretap_path=None, now_epoch_ms=NOW,
    )
...
def test_rtk_float_is_explicit_override():
    ...
    denied = evaluate_preflight(
        rc_host="rc", snapshot=status, mission_trace=trace(),
        wiretap_path=None, now_epoch_ms=NOW,
    )
    allowed = evaluate_preflight(
        rc_host="rc", snapshot=status, mission_trace=trace(), wiretap_path=None,
        policy=PreflightPolicy(allow_rtk_float=True), now_epoch_ms=NOW,
    )
...
def test_platforms_are_not_cross_mapped():
    status = snapshot()
    status["camera"] = {
        **status["camera"], "platform": "M3M", "cameraType": "M3M",
        "surveyProfile": "M3M_RGB_MULTISPECTRAL",
    }
    report = evaluate_preflight(
        rc_host="rc", snapshot=status, mission_trace=trace(), wiretap_path=None,
        expected_platform="M3E", expected_capture_profile="M3E_MAPPING",
        now_epoch_ms=NOW,
    )
    failed = {c.name for c in report.checks if c.status == "FAIL"}
...
def test_stale_accepted_mission_is_no_go(tmp_path):
    ...
    report = evaluate_preflight(
        rc_host="rc", snapshot=snapshot(), mission_trace=mission,
        wiretap_path=wiretap(tmp_path), now_epoch_ms=NOW,
    )
...
def test_wiretap_mismatch_is_no_go(tmp_path):
    ...
    report = evaluate_preflight(
        rc_host="rc", snapshot=snapshot(), mission_trace=trace(),
        wiretap_path=wiretap(tmp_path, wrong), now_epoch_ms=NOW,
    )
    assert any(c.name == "wiretap" and c.status == "FAIL" for c in report.checks)
