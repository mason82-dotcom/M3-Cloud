...
function renderLive(vs){
 ...
 const t=v.telemetry||{}, c=t.controller||{}, a=t.attitude||{}, g=t.gimbal||{}, positioning=v.positioning||{}, r=v.rtkState||{}, s=t.safety||{}, h=v.aircraft?.home||t.home||{}, p=v.payload||{}, platform=String(p.platform||v.model||'UNKNOWN').toUpperCase();
 ...
 <div class="liveGrid">
  ...
  <section class="panel liveSection"><div class="panelHead"><div><h2>GNSS / RTK</h2><small>Fix and reference state</small></div></div><div class="liveMetrics">${metric('Fix',positioning.fix||r.fix||v.rtk)}${metric('Position source',positioning.position_source)}${metric('RTK raw fix',r.raw_fix)}${metric('RTK healthy',r.healthy===true?'yes':r.healthy===false?'no':'—')}${metric('RTK age',r.age_ms,' ms')}${metric('RTK source',r.source)}${metric('GNSS satellites',v.sats)}${metric('RTK satellites',r.satellites??positioning.rtk_satellites)}${metric('Home set',t.home_set===true?'yes':t.home_set===false?'no':'—')}${metric('Home lat',h.latitude)}${metric('Home lon',h.longitude)}</div></section>
